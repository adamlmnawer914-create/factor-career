  ...
  fetch('/data/jobs.json?t=' + Date.now())
    ...
    .then(data => {
      if (Array.isArray(data) && data.length > 0 && window.CareerAI.db) {
        ...
        // If currently on jobs page, re-render smoothly
...
window.CareerAI.pages.jobs = function() {
  ...
  const filteredJobs = allJobs.filter(job => {
    ...
    if (state.type !== 'all') {
      ...
      if (state.type === 'remote' && !type.includes('remote') && !typeAr.includes('?? ???') && !typeAr.includes('?? ????')) return false;
      if (state.type === 'full-time' && !type.includes('full') && !typeAr.includes('????')) return false;
      if (state.type === 'part-time' && !type.includes('part') && !typeAr.includes('????')) return false;
      if (state.type === 'contract' && !type.includes('contract') && !type.includes('freelance') && !typeAr.includes('??') && !typeAr.includes('???')) return false;
    }
      ...
      <div class="container">
        ...
        <div class="text-center" style="max-width:800px;margin:0 auto 2rem">
          <div style="display:inline-flex;align-items:center;gap:8px;background:rgba(99,102,241,0.1);border:1px solid rgba(99,102,241,0.25);border-radius:30px;padding:6px 16px;margin-bottom:1.25rem;">
            ...
            <span style="font-size:0.85rem;font-weight:600;color:var(--color-primary-light);">
              ${isEn ? 'AI-Verified Jobs ? Updated Daily' : '????? ????? ??????? ????????? ? ????? ?????'}
            </span>
          </div>

          <h1 style="font-size:clamp(1.8rem, 4vw, 2.5rem);font-weight:800;color:var(--color-text);line-height:1.25;margin-bottom:1rem">
            ${isEn ? 'Verified Career & Remote Job Opportunities' : '???? ??????? ???? ????? ???????'}
          </h1>
          <p style="font-size:1.05rem;color:var(--color-text-muted);line-height:1.6">
            ${isEn ? 'Explore daily hand-picked and AI-audited job opportunities from verified companies and remote organizations worldwide.' : '?????? ?????? ???? ????? ???????? ?????? ?? ????? ?????? ?????? ??????? ????????? ??????? ?? ????????? ???????.'}
          </p>
        </div>
        ...
        <div class="ad-frame-wrapper ad-frame-banner" style="margin-bottom:2rem;">
          <div class="ad-frame-label">
            ...
            ${isEn ? 'Sponsored Advertisement' : '????? ???? / SPONSORED'}
          </div>
        ...
        <div style="background:var(--color-bg-card);border:1px solid var(--color-border);border-radius:16px;padding:1.5rem;margin-bottom:2.5rem;box-shadow:0 4px 20px rgba(0,0,0,0.08)">
          <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:1rem;align-items:flex-end">
            ...
            <div class="form-group" style="margin:0;grid-column:span 2;">
              <label class="form-label">${isEn ? 'Search by Keyword, Title, Company or Skills' : '????? ??????? ?????????? ??????? ?????? ?? ????????'}</label>
              <input type="text" id="jobsSearchInput" class="form-input" 
                     placeholder="${isEn ? 'e.g. Frontend Developer, Remote, Marketing, Python...' : '????: ????? ???????? ?????? ?? ???? ??????...'}" 
                     value="${state.searchQuery}" 
                     oninput="CareerAI.onJobsSearch(this.value)">
            </div>

            <div class="form-group" style="margin:0">
              <label class="form-label">${isEn ? 'Field / Category' : '???? ????? ???????'}</label>
              <select id="jobsCategoryFilter" class="form-input" onchange="CareerAI.onJobsFilterChange('category', this.value)">
                <option value="all" ${state.category==='all'?'selected':''}>${isEn ? 'All Fields' : '???? ????????'}</option>
                <option value="dev" ${state.category==='dev'?'selected':''}>${isEn ? 'Software & Tech' : '????? ??????????'}</option>
                <option value="marketing" ${state.category==='marketing'?'selected':''}>${isEn ? 'Marketing & Sales' : '????? ???????'}</option>
                <option value="design" ${state.category==='design'?'selected':''}>${isEn ? 'UI/UX & Design' : '????? ?????'}</option>
                <option value="management" ${state.category==='management'?'selected':''}>${isEn ? 'HR & Management' : '????? ????? ??????'}</option>
                <option value="finance" ${state.category==='finance'?'selected':''}>${isEn ? 'Finance & Accounting' : '????? ???????'}</option>
                <option value="support" ${state.category==='support'?'selected':''}>${isEn ? 'Customer Support' : '???? ????? ????'}</option>
              </select>
            </div>

            <div class="form-group" style="margin:0">
              <label class="form-label">${isEn ? 'Work Type' : '??? ?????'}</label>
              <select id="jobsTypeFilter" class="form-input" onchange="CareerAI.onJobsFilterChange('type', this.value)">
                <option value="all" ${state.type==='all'?'selected':''}>${isEn ? 'All Types' : '???? ???????'}</option>
                <option value="remote" ${state.type==='remote'?'selected':''}>${isEn ? 'Remote / Online' : '?? ???? / ???????'}</option>
                <option value="full-time" ${state.type==='full-time'?'selected':''}>${isEn ? 'Full-time' : '???? ????'}</option>
                <option value="part-time" ${state.type==='part-time'?'selected':''}>${isEn ? 'Part-time' : '???? ????'}</option>
                <option value="contract" ${state.type==='contract'?'selected':''}>${isEn ? 'Contract / Freelance' : '??? ?? / ???'}</option>
              </select>
            </div>
          ...
          <div>
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.25rem">
              <h2 style="font-size:1.2rem;font-weight:700;color:var(--color-text)">
                ${isEn ? 'Available Opportunities' : '??????? ???????'} 
                <span style="font-size:0.9rem;font-weight:500;color:var(--color-primary);margin-right:6px">(${filteredJobs.length} ${isEn ? 'jobs' : '????'})</span>
              </h2>
            ...
            ${filteredJobs.length === 0 ? `
              <div class="text-center" style="padding:4rem 1rem;background:var(--color-bg-card);border-radius:16px;border:1px solid var(--color-border)">
                ...
                <h3 style="font-size:1.3rem;font-weight:700;color:var(--color-text);margin-bottom:0.5rem">${isEn ? 'No matching jobs found' : '?? ??? ?????? ??? ????? ??????'}</h3>
                <p style="color:var(--color-text-muted);max-width:420px;margin:0 auto 1.5rem">${isEn ? 'Try adjusting your search terms or clearing filters to see all available jobs.' : '???? ????? ????? ????? ?? ????? ??? ??????? ??????? ??? ???? ????? ???????.'}</p>
                <button class="btn btn--secondary btn--sm" onclick="CareerAI.resetJobsFilter()">${isEn ? 'Reset All Filters' : '????? ??? ???????'}</button>
              </div>
            ` : `
              <div class="jobs-grid" style="display:grid;grid-template-columns:repeat(auto-fit, minmax(300px, 1fr));gap:1.5rem;">
                    ...
                    <div style="padding:1.5rem;flex:1;display:flex;flex-direction:column;">
                      ...
                      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;flex-wrap:wrap;gap:0.5rem">
                        ...
                        <div style="display:flex;gap:0.4rem;flex-wrap:wrap">
                          <span class="job-badge job-badge--type" style="background:rgba(99,102,241,0.15);color:#a5b4fc;border-radius:6px;padding:3px 8px;font-size:0.75rem">${isEn ? (job.type || 'Remote') : (job.type_ar || job.type || '?? ???')}</span>
